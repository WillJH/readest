...
### 1. 陪伴模式(Companion Mode)二期

角色 `mode: 'companion'` 字段已就位(一期)。二期内容:

- 3 个规则触发器:章节切换 / 连续阅读满 N 分钟 / 同页长停留
- 节流上限(每小时最多主动发言 K 次)
- 消息注入到运行中的对话线程(**先写探针验证 assistant-ui 注入 API,这是唯一技术风险点**)
- 阅读器角落气泡通知,点开进 AI 面板
- 每角色可开关,默认关

### 2. 📋 人物卡 + 关系图谱「这是谁来着?」(AI 功能,规格已定,未开工)
