...
- Google Drive 授权(2026-08 二定):**默认全平台同一个官方 client(Android/iOS/桌面 deep-link),保证 `drive.file` 命名空间统一**。血泪教训:`drive.file` 的文件可见性**按 OAuth client 计**——曾因桌面 deep-link 回跳丢失把 Linux 桌面切到自建 Desktop 型 client 的回环授权,结果 Linux 与 Android(官方 client)在 Drive 上各建一个 `Readest/` 文件夹、互不可见、同步彻底劈叉(用户发现"手机和 Linux 不是一个同步"才暴露)。现 `googleDriveConnect.ts` 默认官方 deep-link;回环授权降级为**显式 opt-in 应急通道**——`.env.local` 三行全开才启用(`NEXT_PUBLIC_GOOGLE_LOOPBACK_CLIENT_ID` + `_SECRET` + `NEXT_PUBLIC_GOOGLE_LINUX_LOOPBACK=1`,判定函数 `shouldUseLinuxLoopbackRunner` 在 `buildGoogleDriveProvider.ts`,有测试)。**切换 client 后必须断开重连**:旧 token 属旧 client,新 client 拿去刷新会 invalid_grant;重连后直接落回 Android 那棵 `Readest/` 树合并进度,回环时代建的旧文件夹从此停更、可人工删除。若官方 deep-link 在桌面再回跳丢失,先终端验证 scheme 路由(插件注册本身可靠:`register_all()` 对 AppImage 用 `APPIMAGE` 稳定路径写 Exec + update-desktop-database + xdg-mime default,每次启动执行,见 lib.rs):
  ```bash
  xdg-mime query default x-scheme-handler/com.googleusercontent.apps.209390247301-ctpmep68ppfa56r1b8tr35e4qi4p60kq  # 应答 *-handler.desktop
  xdg-open 'com.googleusercontent.apps.209390247301-ctpmep68ppfa56r1b8tr35e4qi4p60kq:/oauthredirect?test=1'  # 应唤起 Readest(单实例转发,single-instance 事件 args[1]=URL)
  ```
